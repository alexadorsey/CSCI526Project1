...
function Start () {
	...
	if (touchOn) {
		...
		position = GameObject.Find("Position").guiText; //Used for debug and telling status
		position.fontSize = Mathf.Floor(Screen.dpi/7);
		position.text = "";
...
function Update () {
	...
	if (touchOn) {
			...
			// forth/back banking
			transform.localEulerAngles.x = -z * tiltAngle;  //left or right
